inpt = """eb cbgfae cabdf fedab efb adgcef cbgaefd egdb dbgefa eafgd | dfbae be gdafe gcefab
...
inpt2 = """be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
a1 = inpt.split("\n")
a2 = [i.split(" | ")[1] for i in a1]
a22 = [i.split(" | ")[0] for i in a1]
a33= [i.split(" ") for i in a22]
a3 = [i.split(" ") for i in a2]
...
for i in range(len(a2)):
    ...
    if seven == set() or one == set() or four == set():
        print("FUCK!!!", seven, one, four)
        exit()
    a = seven - one
    g = min((i-four)-seven for i in t35)
    e = eight - g - four - seven
    bd = four - one
    d = max(bd - i for i in z69)
    b = four - one - d
    for i in t35:
        if len(i - four) == 2 and len(i - one) == 4:
            f = i - set().union(a,b,d,g)
    c = one - f
    # print()
    ...
    for i in range(4):
        ...
        if set(sec_lst[i]) == set().union(c,f):
            # print((sec_lst[i]))
            final += 1*10**(3-i)
        if set(sec_lst[i]) == set().union(a,b,c,e,f,g):
            # print((sec_lst[i]))
            final += 0*10**(3-i)
        if set(sec_lst[i]) == set().union(a,c,d,e,g):
            # print((sec_lst[i]))
            final += 2*10**(3-i)
        if set(sec_lst[i]) == set().union(a,c,d,f,g):
            # print((sec_lst[i]))
            final += 3*10**(3-i)
        if set(sec_lst[i]) == set().union(b,d,c,f):
            final += 4*10**(3-i)
            # print((sec_lst[i]))
        if set(sec_lst[i]) == set().union(a,c,f):
            # print((sec_lst[i]))
            final += 7*10**(3-i)
        if set(sec_lst[i]) == set().union(a,b,d,f,g):
            # print((sec_lst[i]))
            final += 5*10**(3-i)
        if set(sec_lst[i]) == set().union(a,b,d,e,f,g):
            # print((sec_lst[i]))
            final += 6*10**(3-i)
        if set(sec_lst[i]) == set().union(a,b,c,d,e,f,g):
            # print((sec_lst[i]))
            final += 8*10**(3-i)
        if set(sec_lst[i]) == set().union(a,b,d,c,f,g):
            # print((sec_lst[i]))
            final += 9*10**(3-i)
        #print(1*10**(3-i))
    #print(final)
...
# flat_ls = [item for sublist in a3 for item in sublist]
